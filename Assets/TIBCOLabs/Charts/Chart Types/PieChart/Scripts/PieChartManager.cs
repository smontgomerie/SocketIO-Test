...
{
    [Header("Pie Prefabs")] [Tooltip("this is how each Slice of the Pichart should look like.")]
    ...
    [Header("Pie Label")] [Tooltip("a Label Value shown at each Pie Section.")]
    public string[] PieLabel;

    [Header("Pie Scaling")] [Tooltip("the scale Value define size for each Section.")]
    public float[] PieSize;

    [Header("Pie Colors")] [Tooltip("Color to display for each Section.")]
    public Color[] PieColors;

    private List<GameObject> PieSlices = new List<GameObject>();
    ...
    public void SetPieSize(int index, float value)
    {
        PieSize[index] = value;
        Refresh();
    }
    ...
    public void Refresh()
    {
        ...
        for (int i = 0; i < numberOfSlices; i++)
        {
            ...
            if (100 * i / numberOfSlices < PieSize[slice])
            {
                Renderer rend = PieSlices[i].GetComponent<Renderer>();
                rend.material.SetColor("_SpecColor", PieColors[slice]);
            }
            else
            {
